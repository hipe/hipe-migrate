...
module GemHelpers
  ...
  def generate_gemspec
    ...
    Gem::Specification.new do |s|    
      ...
      s.homepage  = "http://github.com/hipe/hipe-migrate"
      s.date      = %q{2009-11-19}  
      s.summary   = %q{Getopt plus validations and help screen generation}  
      s.description  = <<-EOS.strip
      hipe-migrate is an experimental command-line "framework" that aides
  ...
  def read_gemspec
    @read_gemspec ||= eval(File.read("hipe-migrate.gemspec"))
  end
...
class Default < Thor
  ...
  desc "gemspec", "Regenerate hipe-migrate.gemspec"
  def gemspec
    File.open("hipe-migrate.gemspec", "w") do |file|
    ...
    puts "Wrote gemspec to hipe-migrate.gemspec"
    read_gemspec.validate
  end

  desc "build", "Build a hipe-migrate gem"
  def build
    sh "gem build hipe-migrate.gemspec"
